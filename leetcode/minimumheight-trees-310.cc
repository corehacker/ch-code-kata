...
struct Node {
  ...
  bool visited;
  int level;
  Node() : visited(false), level(0) {}
};

class Solution {
private:
  unordered_map<int, Node *> createGraph(vector<pair<int, int>>& edges) {
    unordered_map<int, Node *> map;
    for(auto &edge : edges) {
        Node *src = nullptr;
        if(!map[edge.first]) {
            map[edge.first] = new Node();
        }
        src = map[edge.first];
        
        Node *dst = nullptr;
        if(!map[edge.second]) {
            map[edge.second] = new Node();
        }
        dst = map[edge.second];
  ...
  int getHeight(Node *root, bool inverted) {
    list<Node *> q;
    root->visited = inverted ? false : true;
    root->level = 0;
      ...
      for(auto &child : curr->children) {
        if(!inverted) {
          if(!child->visited)  {
            ...
            height = child->level > height ? child->level : height;
            child->visited = true;
            q.push_back(child);
          }
        } else {
...
public:
  vector<int> findMinHeightTrees(int n, vector<pair<int, int>>& edges) {
    ...
    unordered_map<int, Node *> graph = createGraph(edges);
    for(int i = 0; i < n; i++) {
      int h = getHeight(graph[i], (i % 2 != 0));
      // cout << "Height of tree rooted @ " << i << ": " << h << endl;
    ...
    for(auto &height : heights) {
      if(height.second < min) {
        min = height.second;
        res.clear();
        res.push_back(height.first);
      } else if (height.second == min) {
        res.push_back(height.first);
      }
    }
    return res;
